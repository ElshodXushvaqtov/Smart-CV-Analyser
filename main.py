    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == '123':
                ...
                ## Pie chart for recommended course
                recommended_course = plot_data.columns[6]
                value_counts_predicted = plot_data[recommended_course].value_counts()
                labels_predicted = value_counts_predicted.index.tolist()
                sizes_predicted = value_counts_predicted.values
                plt.figure(figsize=(10, 6))  # Optional: Adjusts the size of the figure
                plt.pie(sizes_predicted, labels=labels_predicted, autopct='%1.1f%%')
                plt.axis('equal')
                st.subheader("📈 ** Pie-Chart for User's👨‍💻 Recommended Course**")
                st.pyplot(plt)

                ### Pie chart for User's Experienced Level
                experience_level = plot_data.columns[7]
                value_counts_experience = plot_data[experience_level].value_counts()
                labels_experience = value_counts_experience.index.tolist()
                sizes_experience = value_counts_experience.values
                plt.figure(figsize=(10, 6))
                plt.pie(sizes_experience, labels=labels_experience, autopct='%1.1f%%')
                plt.axis('equal')
...
def gpt_survey(content):
    openai.api_key = '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'
